# HMAT for GWAS in which the individual genotype and phenotype can be accessed
# Notes: This function is for HMAT which improves the power of transcriptome wide association studies by 
...
HMAT_individual <- function(y,G2,weight,outcome){
	y <- y
	G2 <- G2
	weight <- weight
	m <- dim(G2)[2]
	...
	p_HMAT = as.vector(c(p.hmp(p_TWAS,L=length(p_TWAS))))
	return(list(p_HMAT=p_HMAT,p_TWAS=p_TWAS))
}

#HMAT for GWAS in which only summary statistics can be accessed
# :::INPUT:::
...
HMAT_summary <- function(Z,G,weight,lambda){
	Z <- as.vector(Z)
	G <- as.matrix(G)
